...
function refreshUI() {
  if (foodContainer.children.length > 0) {
    // Children exist, hide `no-list` div and show statistics
    if (noListEl) noListEl.hidden = true;
    if (foodListStatistics) foodListStatistics.hidden = false;
  } else {
    // No children, show `no-list` div and hide statistics
